module toolsmod.httpsdownloader;
/* This is a version of downs/feep's tools.downloader modified to support HTTPS by opticron/Kinsey Moore.
 * The vast majority of the work here was done by feep.
 */
...
char[] followLink(char[] oldAddr, char[] newAddr) {
  ...
  if (auto rest = oldAddr.startsWith("https://")) oldAddr = rest;
...
TLS!(int) stackguard;
static this() {
  New(download_callbacks); synchronized(download_callbacks) { }
  New(stackguard, { return new int; });
}
...
string download(string param,
  ...
  stackguard() ++;
  scope(exit) stackguard() --;
  if (stackguard() > 16)
    throw new Exception("Stack bust protection triggered: download loop of some kind. ");
  string url=param;
  ...
  string post, refer, cookie, range; bool http_1_0, head, ssl;
  while (true) {
    ...
    if (auto rest=url.startsWith("REFER=")) {
      ...
      if (refer.length<7 || refer[0..7]!="http://" || refer.length<8 || refer[0..8]!="https://") refer="http://"~refer; // referrer needs that
  ...
  if (auto rest=url.startsWith("https://")) {
    url = rest;
    ssl = true;
  }
  ...
  TcpSocket openSocket(string firstMesg, bool ssl = false) {
    TcpSocket sock;
    synchronized (cpoolSync) {
      if (address in cpool && !ssl) {
    ...
    if (!sock) {
      sock= ssl?new TlsSocket:new TcpSocket;
      scope(failure) sock.close;
      sock.blocking=true;// XXX fixme (this might need code in TlsSocket)
      size_t tries=3;
      while (true) {
      ...
      while (firstMesg.length) {
        auto sent = sock.send(firstMesg);
        if (sent <= 0) throw new Exception("Cannot send first message! ");
        firstMesg = firstMesg[sent .. $];
      }
    }
    return sock;
  }
...
retry:
  if (!retries) {
    if (param.length > 100) param = param[0 .. 100]~"[...]";
    throw new Exception("Can't download page: "~param);
  }
  retries--;
  ...
  bool checkHeader(ref bool chunked, out string res) {
    ...
    else if (head) { res = received; return true; }
    hpos += 4;
  ...
  if (reuseConnection && !ssl) synchronized (cpoolSync) if (address in cpool) cpool[address]~=sock; else cpool[address]=[sock];
  if (chunked) {
    if (!chunks.length) {
